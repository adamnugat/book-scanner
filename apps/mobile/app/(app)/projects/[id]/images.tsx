...
export default function ProjectImagesScreen() {
  ...
  const uploadAssets = async (assets: ImagePicker.ImagePickerAsset[]) => {
    const files = assets.map((a, i) => ({
      uri: a.uri,
      name: a.fileName || `page-${Date.now()}-${i}.jpg`,
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const result = await api.uploadImages(id, [files[i]]);
    ...
    const errCount = allNewImages.length < files.length ? files.length - allNewImages.length : 0;
    if (errCount > 0) {
      showToast(`Przesłano ${allNewImages.length}/${files.length} plików`, 'error');
    } else {
  ...
  const moveImage = async (index: number, direction: -1 | 1) => {
    ...
    setImages(newOrder);
    try {
      await api.reorderImages(id, newOrder.map((i) => i.id));
    } catch {
      setImages(images);
    }
  };
  ...
  const dropZoneProps = Platform.OS === 'web' ? {
    onDragOver: (e: { preventDefault: () => void }) => { e.preventDefault(); setIsDragOver(true); },
      ...
      {images.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>
            {Platform.OS === 'web' ? 'Przeciągnij pliki lub kliknij „Galeria"' : 'Dodaj zdjęcia stron książki'}
          </Text>
        </View>
      ) : (
        <FlatList
            ...
            <View style={styles.progressList}>
              ...
              {fileProgress.map((fp, i) => (
                <View key={i} style={styles.progressItem}>
                  <Text style={styles.progressName} numberOfLines={1}>{fp.name}</Text>
                  <Text style={[styles.progressStatus, {
                    color: fp.status === 'done' ? '#06d6a0' : fp.status === 'error' ? '#e94560' : fp.status === 'uploading' ? '#f0a500' : '#888',
                  }]}>
                    {fp.status === 'done' ? '✓' : fp.status === 'error' ? '✗' : fp.status === 'uploading' ? '↑' : '•'}
                  </Text>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  card: {
    backgroundColor: '#16213e', borderRadius: 10, padding: 12, marginBottom: 10,
    borderWidth: 1, borderColor: '#0f3460',
  },
  cardRow: { flexDirection: 'row', alignItems: 'center' },
  pageNum: { color: '#888', fontSize: 16, fontWeight: 'bold', width: 28, textAlign: 'center' },
  thumb: { width: 60, height: 80, borderRadius: 6, backgroundColor: '#0f3460', marginHorizontal: 10 },
  cardInfo: { flex: 1 },
  ...
  moveBtn: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 6, backgroundColor: '#0f3460' },
  ...
  progressList: { backgroundColor: '#16213e', borderRadius: 12, padding: 16, width: '80%', maxWidth: 400 },
  progressItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 },
  progressName: { color: '#e0e0e0', fontSize: 13, flex: 1, marginRight: 8 },
  progressStatus: { fontSize: 16, fontWeight: 'bold' },
  dropOverlay: {
    ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(233,69,96,0.15)',
    alignItems: 'center', justifyContent: 'center', zIndex: 10,
    borderWidth: 3, borderColor: '#e94560', borderStyle: 'dashed', borderRadius: 12, margin: 8,
  },
  dropOverlayText: { color: '#e94560', fontSize: 20, fontWeight: 'bold' },
  bottomBar: {
    position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row',
    padding: 16, gap: 12, backgroundColor: '#1a1a2e', borderTopWidth: 1, borderTopColor: '#0f3460',
  },
  addBtn: {
    flex: 1, backgroundColor: '#e94560', borderRadius: 8, padding: 14, alignItems: 'center',
  },
  addBtnText: { color: '#fff', fontSize: 15, fontWeight: '600' },
  nextBtn: {
    flex: 1, backgroundColor: '#06d6a0', borderRadius: 8, padding: 14, alignItems: 'center',
  },
