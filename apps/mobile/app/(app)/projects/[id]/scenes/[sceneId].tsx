...
export default function SceneEditorScreen() {
  ...
  const [scene, setScene] = useState<(SceneResponse & { pageImage: { storagePath: string; thumbnailPath: string | null; imageUrl: string; thumbnailUrl: string | null; originalFilename: string | null } }) | null>(null);
  ...
  const imagePreview = (
    <View style={[styles.imagePanel, isDesktop && styles.imagePanelDesktop]}>
      <Image
        source={{ uri: scene.pageImage.thumbnailUrl || scene.pageImage.imageUrl }}
        style={styles.previewImage}
        resizeMode="contain"
      />
      <Text style={styles.imageName}>{scene.pageImage.originalFilename || `Strona ${scene.orderIndex + 1}`}</Text>
    </View>
  );
  ...
  const editor = (
    <View style={[styles.editorPanel, isDesktop && styles.editorPanelDesktop]}>
      <View style={styles.editorHeader}>
        <Text style={styles.sceneTitle}>Scena {scene.orderIndex + 1} z {allScenes.length}</Text>
        <Text style={[styles.statusBadge, { color: scene.status === 'ready_for_audio' ? '#06d6a0' : '#f0a500' }]}>
      ...
      <TextInput
        ...
        multiline
        value={editedText}
        onChangeText={(text) => { setEditedText(text); setDirty(true); }}
      ...
      <View style={styles.counter}>
        <Text style={styles.counterText}>{wordCount} słów · {charCount} znaków</Text>
    ...
    <View style={styles.container}>
          ...
          <ScrollView style={styles.sidePanel}>
            {allScenes.map((s) => (
              ...
              >
                ...
                <Text style={[styles.sideItemText, s.id === sceneId && styles.sideItemTextActive]} numberOfLines={1}>
      ...
      <View style={styles.navBar}>
        <Pressable
          ...
          disabled={!prevScene}
        >
          <Text style={[styles.navBtnText, !prevScene && styles.navBtnTextDisabled]}>← Poprzednia</Text>
        </Pressable>
        <Pressable
          ...
          disabled={!nextScene}
        >
          <Text style={[styles.navBtnText, !nextScene && styles.navBtnTextDisabled]}>Następna →</Text>
        </Pressable>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  sidePanel: { width: 260, borderRightWidth: 1, borderRightColor: '#0f3460', backgroundColor: '#16213e' },
  sideItem: { flexDirection: 'row', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: '#0f3460' },
  ...
  textArea: {
    backgroundColor: '#16213e', color: '#e0e0e0', borderRadius: 8, padding: 16,
    fontSize: 15, lineHeight: 24, minHeight: 200, borderWidth: 1, borderColor: '#0f3460',
  },
  ...
  saveBtn: { backgroundColor: '#e94560', borderRadius: 8, padding: 14, alignItems: 'center', marginTop: 12 },
  saveBtnText: { color: '#fff', fontSize: 15, fontWeight: '600' },

  navBar: {
    flexDirection: 'row', justifyContent: 'space-between', padding: 16, gap: 12,
    borderTopWidth: 1, borderTopColor: '#0f3460', backgroundColor: '#1a1a2e',
  },
  navBtn: { flex: 1, padding: 12, borderRadius: 8, borderWidth: 1, borderColor: '#0f3460', alignItems: 'center' },
