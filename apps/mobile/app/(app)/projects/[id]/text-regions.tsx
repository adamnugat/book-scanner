...
export default function TextRegionsScreen() {
  ...
  const addRegion = (imageId: string) => {
    setRegions((prev) => [
      ...prev,
      { key: `${imageId}-${Date.now()}`, pageImageId: imageId, x: 0, y: 0, width: 100, height: 100 },
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await api.saveTextRegions(
        id,
        regions.map(({ pageImageId, x, y, width, height }) => ({ pageImageId, x, y, width, height })),
      );
  ...
  return (
    <View style={styles.container}>
          ...
          return (
            <View style={styles.imageCard}>
              <View style={styles.imageHeader}>
                <Image source={{ uri: item.thumbnailUrl || item.imageUrl }} style={styles.thumb} />
      ...
      <View style={styles.bottomBar}>
        <Pressable style={styles.skipBtn} onPress={() => router.push(`/(app)/projects/${id}/scenes`)}>
...
const styles = StyleSheet.create({
  ...
  imageCard: {
    backgroundColor: '#16213e', borderRadius: 10, padding: 12, marginBottom: 12,
  ...
  addRegionBtn: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, backgroundColor: '#0f3460' },
  ...
  fieldInput: {
    backgroundColor: '#1a1a2e', color: '#e0e0e0', borderRadius: 4, padding: 6,
    fontSize: 13, borderWidth: 1, borderColor: '#0f3460', textAlign: 'center',
  },
  ...
  skipBtn: { flex: 1, borderRadius: 8, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: '#0f3460' },
  skipBtnText: { color: '#888', fontSize: 15 },
  saveBtn: { flex: 2, backgroundColor: '#e94560', borderRadius: 8, padding: 14, alignItems: 'center' },
