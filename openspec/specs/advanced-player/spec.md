...
- **AND** SHALL nakładać gradient opacity na górze i na dole, `pointerEvents="none"`
...
- **THEN** `SceneTranscriptBox` SHALL NOT przewijać tekstu
...
- **THEN** tekst SHALL być przewijany liniowo wraz z postępem odtwarzania
...
- **WHEN** użytkownik skoczy do innej sceny lub scena zmieni się automatycznie
- **THEN** `SceneTranscriptBox` SHALL zresetować pozycję do początku i ponownie zmierzyć wysokość nowego tekstu

#### Scenario: Pauza odtwarzania
- **WHEN** użytkownik wstrzyma odtwarzanie
- **THEN** przewijanie transkrypcji SHALL zatrzymać się w bieżącej pozycji

---

### Requirement: Informacje o scenie i lista scen
Ekran zaawansowanego odtwarzacza SHALL wyświetlać informacje o aktualnie odtwarzanej scenie oraz listę wszystkich scen z możliwością skoku.
...
- **THEN** ekran SHALL wyświetlać listę scen z numerem, fragmentem `sceneText` i czasem trwania
- **AND** bieżąca scena SHALL mieć aktywne wyróżnienie tokenami `audioFlowTokens.color.accent.pearl`

#### Scenario: Skok do sceny przez tap
- **WHEN** użytkownik tapnie element listy scen
- **THEN** system SHALL wywołać `jumpToScene(item.sceneOrderIndex ?? 0)`
- **AND** `SceneTranscriptBox` SHALL zresetować się i pokazać tekst nowej sceny

---

### Requirement: Sekcja pobierania offline
Ekran zaawansowanego odtwarzacza SHALL eksponować sekcję pobierania offline z trzema stanami: nie pobrane, w trakcie pobierania, pobrane.

#### Scenario: Stan nie pobrane — online
- **WHEN** `!isCached && !downloading && isOnline`
- **THEN** sekcja SHALL renderować przycisk "Pobierz offline" wywołujący `handleDownloadOffline()`
...
- **WHEN** `isCached === true`
- **THEN** sekcja SHALL renderować tekst `✓ Offline (X.X MB)` oraz przycisk "Usuń cache" wywołujący `handleDeleteCache()`

#### Scenario: Stan nie pobrane — offline
- **WHEN** `!isCached && !downloading && !isOnline`
- **THEN** sekcja SHALL ukryć przycisk pobierania

---

### Requirement: Banner trybu offline
Ekran zaawansowanego odtwarzacza SHALL wyświetlać banner informacyjny gdy aktywny jest tryb offline lub brak połączenia.

#### Scenario: Banner przy odtwarzaniu z cache
- **WHEN** `isOfflineMode === true`
- **THEN** ekran SHALL wyświetlać banner z tekstem `📴 Odtwarzanie z cache (offline)`

#### Scenario: Banner przy braku połączenia
...
Ekran zaawansowanego odtwarzacza SHALL obsługiwać stan ładowania i pustej playlisty w stylu AudioFlow.

#### Scenario: Stan loading
- **WHEN** `loading === true`
- **THEN** ekran SHALL renderować `AudioFlowScreen` z `<ActivityIndicator>` wycentrowanym w kolorze `audioFlowTokens.color.accent.*`

#### Scenario: Stan empty
- **WHEN** `loading === false && playlist.length === 0`
- **THEN** ekran SHALL renderować `AudioFlowScreen` z tekstem `Brak audio. Wygeneruj audio ze scen.`
